...
export class ArenderService {
  ...
  openInWindow(nodes: Array<MinimalNodeEntity>) {
    ...
    if (!onPromise) {
      ...
      from(
        this.apiService
          .getInstance()
          .webScript.executeWebScript('GET', 'slingshot/doclib/action/arenderUpload', { nodeRef: 'workspace://SpacesStore/' + id }, 'alfresco', 's')
      ).subscribe(
        (webScriptdata) => {
          uuid = webScriptdata['UUID'];
          this.ecmUserService.getCurrentUserInfo().subscribe((u) => {
            userName = u.id;
            url = this.buildArenderURLString(uuid, userName, null, null, false, documentbuilder, isFolder);
      ...
      if (isFolder) {
        this.ecmUserService.getCurrentUserInfo().subscribe((u) => {
          ...
          this.clearNodeCache(id, userName, versionLabel).subscribe(
            () => {},
            () => {}
          );
          url = this.buildArenderURLString(id, userName, alfTicket, null, true, false, isFolder);
          window.open(url.toString(), '_blank');
        });
      } else {
        this.apiService
          .getInstance()
          .webScript.executeWebScript('GET', 'api/version', { nodeRef: 'workspace://SpacesStore/' + id }, 'alfresco', 's')
          .then(
            (webScriptdata) => {
              ...
              this.ecmUserService.getCurrentUserInfo().subscribe((u) => {
                ...
                url = this.buildArenderURLString(id, userName, alfTicket, versionLabel, true, documentbuilder, isFolder);
                window.open(url.toString(), '_blank');
              });
            },
            () => {}
  ...
  compare(nodes: Array<MinimalNodeEntity>) {
    ...
    this.ecmUserService.getCurrentUserInfo().subscribe((u) => {
      ...
      const versions = forkJoin(this.getVersion(nodes[0]), this.getVersion(nodes[1]));

      versions.subscribe((data) => {
        url += 'workspace://SpacesStore/' + this.getDocId(nodes[0].entry) + ';' + data[0][0]['label'] + ',';
        url += 'workspace://SpacesStore/' + this.getDocId(nodes[1].entry) + ';' + data[1][0]['label'];
        url += '&visualization.multiView.doComparison=true&user=' + userName + '&alf_ticket=' + alfTicket;
        window.open(url.toString(), '_blank');
      });
    });
  }
  ...
  getVersion(node: MinimalNodeEntity): Observable<any> {
    return from(
      this.apiService
        .getInstance()
        .webScript.executeWebScript('GET', 'api/version', { nodeRef: 'workspace://SpacesStore/' + this.getDocId(node.entry) }, 'alfresco', 's')
    );
  }

  clearNodeCache(nodeId: string, userName: string, versionLabel: string): Observable<Object> {
    const arenderHost = this.appConfig.get<string>('arender.host');
    const args = 'nodeRef=workspace://SpacesStore/' + nodeId + '&user=' + userName + '&versionLabel=' + versionLabel;
    ...
    return this.http.get(arenderHost + 'evictDocument.jsp?uuid=*' + nodeRefB64 + '*', httpOptions);
  }
  ...
  getFolderUUID(nodes: Array<MinimalNodeEntity>, userName: string, version: any, alf_ticket: string): Observable<Object> {
    const arenderHost = this.appConfig.get<string>('arender.host');
    const queryUrls = [];
    ...
    for (let i = 0; i < nodes.length; i++) {
      queryUrls.push({
        queryUrl:
          'loadingQuery' +
          this.buildArenderURLParameters(
  ...
  buildArenderURL(
    ...
    return this.sanitizer.bypassSecurityTrustResourceUrl(
      this.buildArenderURLString(nodeId, userName, alfTicket, versionLabel, onPromise, documentbuilder, isFolder)
    );
  }
  ...
  buildArenderURLString(
    ...
    const arenderHost = this.appConfig.get<string>('arender.host');
    const arenderURL = arenderHost + this.buildArenderURLParameters(nodeId, userName, alfTicket, versionLabel, onPromise, documentbuilder, isFolder);
    return arenderURL;
  }
}
